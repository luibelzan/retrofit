<template>
  <div class="container mt-3 border">
    ...
    <h1 class="text-center mb-1 mt-3">Proceso de Recuperados</h1>
    <!-- Contador de contadores reparados -->
    <div v-if="contadores.length > 0" class="contador-recepcionado text-center mt-4">
      <h3 class="contador-numero">Contadores Reparados: {{ contadores.length }}</h3>
  ...
  methods: {
    async fetchData() {
      try {
        ...
        const resDiagnosticos = await fetch("http://localhost:8080/api/achatarrado/diagnosticosRP");
    ...
    async enviarContadores() {
      try {
        ...
        Swal.fire({
          ...
          text: "Añadido correctamente los contadores recuperados.",
        });
        this.contadores = [];
      } catch (error) {
        Swal.fire({
